...
module.exports = class Endpoint {
	constructor(client, endpointName) {
		...
		if (!['base', 'oauth'].includes(endpointName) && client === undefined) {
			throw new Error(
				`[${endpointName.toUpperCase()}] Client must be defined.`
			);
		}
	...
	async get(query = '') {
		try {
			...
			// console.log(err);
			throw new Error(
				`${err.response.status} error: GET ${this.endpoint + query} -> ${
					this._statuses[err.response.status]
				}`
			);
		}
	}
	...
	async post(query = '', data = {}) {
		try {
			...
			throw new Error(
				`${err.response.status} error: POST ${this.endpoint + query} -> ${
					this._statuses[err.response.status]
				}`
			);
		}
	...
	async patch(query = '', data = {}) {
		try {
			...
			throw new Error(
				`${err.response.status} error: PATCH ${this.endpoint + query} -> ${
					this._statuses[err.response.status]
				}`
			);
		}
	...
	async delete(query = '') {
		try {
			...
			throw new Error(
				`${err.response.status} error: DELETE ${this.endpoint + query} -> ${
					this._statuses[err.response.status]
				}`
			);
		}
	...
	async put(query = '', data = {}) {
		try {
			...
			throw new Error(
				`${err.response.status} error: PUT ${this.endpoint + query} -> ${
					this._statuses[err.response.status]
				}`
			);
		}
