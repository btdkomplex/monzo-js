...
module.exports = class Pots extends Endpoint {
	...
	/**
	 * Deposit the specified amount to the speified pot.
	 * @param {string} accountId The account Id to deposit from.
	 * @param {string} potId The id of the pot to deposit to..
	 * @param {integer} amount The amount to deposit.
	 */
	async deposit(accountId, potId, amount) {
		try {
			...
			return await this.put(potId + '/deposit', data);
		} catch (err) {
			console.log(err.message);
		}
	}
